(function () {
    ...
    window.OsWidget = {
        ...
        rollbackToOriginalText: function (defaultLanguage) {
            return OsWidget.webTranslation(defaultLanguage);
        },

        isTextTranslatable: function (text) {
            var t = text.trim();
            if (!t) return false;
            if (t.charAt(0) === '<') return false;
            if (t.includes("'http") || t.includes('"http')) return false;
            if (t.includes("window.")) return false;
            if (t.replace(/\s/g, "").includes("=window")) return false;
            return true;
        },

        /**
         * Get original nodeValue from text nodes
         */
        loadOriginalTextNodes: function (body) {
            var n, walk = document.createTreeWalker(body, NodeFilter.SHOW_TEXT, null, false);
            var textSegmentLimit = 128;
            var textSegmentCount = 0;
            while (n = walk.nextNode()) {
                if (textSegmentCount == textSegmentLimit) break;
                if (OsWidget.isTextTranslatable(n.nodeValue)) {
                    OsWidget.originalTextNodes.push(n);
                    textSegmentCount++;
                }
            }
        },

        loadOriginalText: function () {
            var defaultLanguage = window.OsSelectors[0].defaultValue;
            var originalTextNodes = OsWidget.originalTextNodes;

            Object.keys(originalTextNodes).map(function (objectKey) {
                var text = originalTextNodes[objectKey].nodeValue;
                if (text.length > 0) OsWidget.sourceTexts.push(text);
            });

            OsWidget.translates[defaultLanguage] = OsWidget.sourceTexts.map(function(val) {
                var m = {};
                m[val] = val;
                return m;
            });
        ...
        // called when the widget is initiated
        webTranslation: function (targetLanguageId) {
            var textNodes = OsWidget.originalTextNodes;

            /**
             * JSON Object
             */
            var jsonObj = {
                "sourceTexts": OsWidget.sourceTexts,
                "targetLanguageId": targetLanguageId
            }

            /**
             * Replace text to translated
             */
            function replaceText(data) {
                Object.keys(textNodes).forEach(function (objectKey, index) {
                    textNodes[objectKey].nodeValue = Object.values(data[index])[0].replace(/^\s+|\s+$/gm, ' ');
                });
            }
